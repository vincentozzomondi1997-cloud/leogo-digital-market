...
(function(){
  ...
  function shell(title,body){
    ...
    modal.innerHTML='<div style="width:min(560px,100%);max-height:92vh;overflow:auto;background:#fff;border-radius:22px;padding:24px;box-shadow:0 25px 80px rgba(0,0,0,.3)">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:16px"><h2 style="margin:0;color:#07152f">'+esc(title)+'</h2><button type="button" data-pb-close style="border:0;background:#eef3fb;border-radius:10px;width:38px;height:38px;font-size:20px">×</button></div>'+body+'</div>';
  ...
  async function getCurrent(){
    ...
    let payment=null;
    if(membership){
      const p=await sb.from('premium_payments').select('id,membership_id,amount,reference,status,created_at').eq('membership_id',membership.id).order('created_at',{ascending:false}).limit(1).maybeSingle();
      if(!p.error)payment=p.data||null;
    }
  ...
  function paymentForm(membership,payment){
    const hasPending=payment&&payment.status==='pending';
    const reference=payment?.reference||'';
    return '<div style="padding:12px 14px;border-radius:12px;background:#fff7ed;color:#9a3412;font-size:13px;margin-bottom:14px">'+
      '<b>Pay by M-Pesa Till: 5494825</b><br>Send exactly <b>'+money(membership.price)+'</b>, then enter the M-Pesa transaction reference below. Your payment will remain pending until an admin verifies it.</div>'+
      '<div style="padding:14px;border:1px solid #e5e7eb;border-radius:14px;margin-bottom:14px"><b>'+esc(membership.plan_name)+'</b><br><span style="font-size:24px;font-weight:900;color:#07152f">'+money(membership.price)+'</span><br><span style="color:#667085">'+esc(membership.duration_days)+' days Premium access</span></div>'+
      '<label style="display:block;font-size:13px;font-weight:800;margin-bottom:6px">M-Pesa Transaction Reference</label>'+ 
      '<input id="pbReference" maxlength="40" value="'+esc(reference)+'" placeholder="e.g. QGH12356UY" style="width:100%;box-sizing:border-box;border:1px solid #d7dce4;border-radius:11px;padding:12px;outline:none;text-transform:uppercase">'+
      '<div id="pbMsg" style="margin-top:10px;font-size:13px"></div>'+ 
      '<button id="pbSubmit" type="button" style="width:100%;border:0;border-radius:12px;padding:13px;background:#ff7a00;color:#fff;font-weight:900;margin-top:10px">'+(hasPending?'UPDATE PAYMENT REFERENCE':'I HAVE PAID — SUBMIT FOR APPROVAL')+'</button>';
  }

  async function showPayment(membership,payment){
    const m=shell('Premium Payment',paymentForm(membership,payment));
    const input=m.querySelector('#pbReference');
    const msg=m.querySelector('#pbMsg');
    m.querySelector('#pbSubmit').onclick=async()=>{
      ...
      const btn=m.querySelector('#pbSubmit');btn.disabled=true;btn.textContent='Submitting…';
      const q=await sb.rpc('submit_premium_payment',{p_membership_id:membership.id,p_reference:ref});
      if(q.error){msg.innerHTML='<span style="color:#991b1b">'+esc(q.error.message)+'</span>';btn.disabled=false;btn.textContent=payment?'UPDATE PAYMENT REFERENCE':'I HAVE PAID — SUBMIT FOR APPROVAL';return;}
      m.querySelector('div[style*="padding:12px"]').innerHTML='<b>Payment submitted successfully.</b><br>Your reference <b>'+esc(ref)+'</b> has been sent to LEOGO Admin for verification. Premium access will activate only after approval.';
      input.disabled=true;btn.disabled=true;btn.textContent='PAYMENT SENT TO ADMIN';
    };
  }
  ...
  async function choose(plan){
    ...
    if(current.membership?.status==='pending'){
      return showPayment(current.membership,current.payment);
    }
  ...
  async function open(){
    try{
      ...
      if(active){
        const expires=new Date(current.membership.expires_at).toLocaleString();
        shell('Premium Membership','<div style="padding:14px;border-radius:12px;background:#ecfdf3;color:#166534"><b>Premium is active.</b><br>Expires: '+esc(expires)+'</div>');
        return;
      }
  ...
  function intercept(e){
    ...
    if(t==='CHOOSE MEMBERSHIP' || t==='CHOOSE PLAN' || t==='MEMBERSHIP'){
      ...
      if(t==='MEMBERSHIP' || t==='CHOOSE MEMBERSHIP')open();
    }
  }
