/* LEOGO TRANSPORT QUOTE WORKFLOW
 * Isolated workflow layer. Existing transport dashboard functions remain intact.
 */
(function(){
  ...
  let transportDomUpdating=false;
  ...
  function syncTransportActions(box,r){
    ...
    if(!canAccept)acceptButton.title='Customer must accept the transport price before this assignment can be accepted.';
    else acceptButton.removeAttribute('title');
  }
  async function transporterRefresh(){
    const body=document.getElementById('tdBookingRows');if(!body)return;
    const boxes=[...body.querySelectorAll('[data-book-actions]')];if(!boxes.length)return;
    const ids=boxes.map(x=>x.getAttribute('data-book-actions')).filter(Boolean);if(!ids.length)return;
    const q=await sb.from('transport_requests').select('id,status,price,service_fee,customer_total,quote_status').in('id',ids);if(q.error)return;
    const map=new Map((q.data||[]).map(x=>[x.id,x]));transportDomUpdating=true;
    try{
      boxes.forEach(box=>{
        const r=map.get(box.getAttribute('data-book-actions'));if(!r)return;
        box.querySelectorAll('[data-lq]').forEach(x=>x.remove());
        ...
        if(s==='assigned'&&(qs==='awaiting_rider_quote'||qs==='rejected')){
          ...
          const x=document.createElement('span');x.className='lq lq-ok';x.style.cssText='margin:0;padding:7px 10px';x.setAttribute('data-lq','1');x.textContent='Customer accepted · '+money(r.customer_total);box.insertBefore(x,box.firstChild);
        }
      });
    }finally{transportDomUpdating=false}
  }
  function blockTransportStatusChanges(){
    document.addEventListener('click',async e=>{
      const b=e.target.closest('#tdBookingRows button[data-status]');if(!b)return;
      e.preventDefault();e.stopImmediatePropagation();
      const box=b.closest('[data-book-actions]'),id=box?.getAttribute('data-book-actions'),st=b.getAttribute('data-status');if(!id)return;
      if(st==='In Progress'){
        const q=await sb.from('transport_requests').select('status,quote_status').eq('id',id).maybeSingle();
        if(q.error||q.data?.quote_status!=='accepted'||String(q.data?.status).toLowerCase()!=='accepted'){
          alert('The customer must accept the transport price before transportation can start.');scheduleTransporterRefresh();return;
        }
      }
      b.disabled=true;
      const q=await sb.rpc('transporter_update_transport_status',{p_request_id:id,p_status:st});
      if(q.error){alert(q.error.message);b.disabled=false;scheduleTransporterRefresh();return}
      scheduleTransporterRefresh();
    },true);
  }
  function transporterBoot(){
    const body=document.getElementById('tdBookingRows');
    if(!body){setTimeout(transporterBoot,500);return}
    css();transporterRefresh();
    const observer=new MutationObserver(()=>{if(!transportDomUpdating)scheduleTransporterRefresh()});
    observer.observe(body,{childList:true,subtree:true});
    blockTransportStatusChanges();
  }
  async function customerRefresh(){
    const area=document.getElementById('leogoCustomerTransportArea');if(!area)return;
    const s=await sb.auth.getSession(),uid=s.data?.session?.user?.id;if(!uid)return;
    const q=await sb.from('transport_requests').select('id,pickup_location,destination,status,price,service_fee,customer_total,quote_status').eq('customer_id',uid).order('created_at',{ascending:false}).limit(100);if(q.error)return;
    let host=document.getElementById('lqCustomer');
  ...
  async function adminRefresh(){
    const area=document.getElementById('leogoTransportInlineArea');if(!area)return;css();
    const selects=[...area.querySelectorAll('[id^="transportAssign_"]')];
    for(const sel of selects){
      ...
      box.querySelector('[data-admin-save]').onclick=async()=>{
        ...
        const b=box.querySelector('[data-admin-save]');b.disabled=true;b.textContent='SAVING…';
        const x=await sb.rpc('admin_set_transport_price',{p_request_id:id,p_price:n});
        if(x.error){msg.textContent=x.error.message;b.disabled=false;b.textContent='UPDATE PRICE';return}
        msg.textContent='Updated. Customer total: '+money(x.data?.customer_total||n*1.1)+'. Customer acceptance is required again.';b.textContent='UPDATED';
      };
    }
  }
  function adminBoot(){css();adminRefresh();setInterval(adminRefresh,3000)}
  function boot(){
    const a=!!document.getElementById('tdBookingRows'),b=!!document.getElementById('leogoCustomerTransportArea'),c=!!document.getElementById('leogoTransportInlineArea');
    if(a)transporterBoot();if(b)customerBoot();if(c)adminBoot();else setTimeout(boot,1000);
  }
