/* LEOGO ADMIN DISPATCH - isolated assignment controls.
   Adds admin-only assignment panels without replacing existing admin functions. */
(function(){
  ...
  async function loadServiceDispatch(){
    ...
    const area=document.getElementById('leogoServiceDispatchPanelArea');
    if(!(await sessionOk())){area.innerHTML='<div class="empty">Admin login required.</div>';return;}
    area.innerHTML='<div class="empty">Loading service bookings…</div>';
    ...
    const av=await sb.from('services').select('provider_id,name,category').eq('approved',true).eq('available',true);
    const rows=av.data||[];
    ...
    area.innerHTML=jobs.map(job=>{
      const service=sm.get(job.service_id)||{};const customer=cm.get(job.customer_id)||{};const current=job.provider_id?pm.get(job.provider_id):null;
  ...
  window.__leogoAssignService=async function(jobId,btn){
    const sel=document.getElementById('svcAssign_'+jobId);const providerId=sel?.value;if(!providerId)return;
    if(btn){btn.disabled=true;btn.textContent='Saving…';}
    const q=await sb.from('service_jobs').update({provider_id:providerId,status:'Assigned',updated_at:new Date().toISOString()}).eq('id',jobId);
    if(q.error){alert(q.error.message);if(btn){btn.disabled=false;btn.textContent='ASSIGN';}return;}
    await loadServiceDispatch();
  };

  async function loadTransportDispatch(){
    const page=document.getElementById('page-transport');if(!page)return;
    const p=card('🚚 Transport Booking Assignment','Assign transport requests to an approved, available driver and vehicle.', 'leogoTransportDispatchPanel');
    if(!p.parentNode)page.appendChild(p);
    document.getElementById('leogoTransportDispatchRefresh').onclick=loadTransportDispatch;
    const area=document.getElementById('leogoTransportDispatchPanelArea');
    if(!(await sessionOk())){area.innerHTML='<div class="empty">Admin login required.</div>';return;}
    ...
    if(vq.error){area.innerHTML='<div class="notice error">'+esc(vq.error.message)+'</div>';return;}
  ...
  window.__leogoAssignTransport=async function(requestId,btn){
    const sel=document.getElementById('transportAssign_'+requestId);const vehicleId=sel?.value;if(!vehicleId)return;
    const v=await sb.from('vehicles').select('owner_id').eq('id',vehicleId).maybeSingle();if(v.error||!v.data?.owner_id){alert(v.error?.message||'Could not identify the vehicle owner.');return;}
    if(btn){btn.disabled=true;btn.textContent='Saving…';}
  ...
  async function loadDeliveryDispatch(){
    ...
    const p=card('📦 Delivery Assignment','Assign customer product orders to an approved and available delivery provider.', 'leogoDeliveryDispatchPanel');
    if(!p.parentNode)page.appendChild(p);
    document.getElementById('leogoDeliveryDispatchRefresh').onclick=loadDeliveryDispatch;
    const area=document.getElementById('leogoDeliveryDispatchPanelArea');
    if(!(await sessionOk())){area.innerHTML='<div class="empty">Admin login required.</div>';return;}
    area.innerHTML='<div class="empty">Loading deliveries…</div>';
    ...
    if(oq.error){area.innerHTML='<div class="notice error">'+esc(oq.error.message)+'</div>';return;}
    if(vq.error){area.innerHTML='<div class="notice error">'+esc(vq.error.message)+'</div>';return;}
    const orders=oq.data||[],vehicles=vq.data||[];
    if(!orders.length){area.innerHTML='<div class="empty">No unassigned deliveries waiting.</div>';return;}
  ...
  window.__leogoAssignDelivery=async function(orderId,btn){
    const riderId=document.getElementById('deliveryAssign_'+orderId)?.value;if(!riderId)return;
    if(btn){btn.disabled=true;btn.textContent='Saving…';}
    const q=await sb.from('orders').update({assigned_rider_id:riderId,updated_at:new Date().toISOString()}).eq('id',orderId);
    if(q.error){alert(q.error.message);if(btn){btn.disabled=false;btn.textContent='ASSIGN DELIVERY';}return;}
    await loadDeliveryDispatch();
  };
  ...
  function boot(){
    ...
    if(s)loadServiceDispatch();if(t)loadTransportDispatch();if(d)loadDeliveryDispatch();
  }
  const timer=setInterval(()=>{
    const exists=document.getElementById('page-services')||document.getElementById('page-transport')||document.getElementById('page-delivery');
    if(exists)boot();
  },800);
